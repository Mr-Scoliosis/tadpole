...
class Project:
    ...
    def create(self):
        ...
        if self.textbox.get("1.0", END) == " ":
            print("problem")
            return
        self.button = Button(self.canvas, text=self.textbox.get("1.0", END), command=self.show_Ponds)
    ...
    def show_Ponds(self):
        ...
        for i in self.ponds:
            print(i)
        createPond = Pond("Name", self.canvas)
...
class TadPole():
    def __init__(self, root):
        ...
        self.User2.grid(row=1,column=3, padx=10, pady=10)
    ...
    def leader(self):
        self.User1.destroy()
        self.User2.destroy()
        self.Projects.append(Project(self.frame))
